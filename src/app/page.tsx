...
export default function Home() {
  ...
  // Toggle bottom bar when hero image scrolls into view
  useEffect(() => {
    const onScroll = () => {
      const el = heroImageRef.current;
      if (!el) return;
      const rect = el.getBoundingClientRect();
      setShowBottomBar(rect.top < window.innerHeight * 0.5);
    };
    window.addEventListener('scroll', onScroll, { passive: true });
    onScroll();
    return () => window.removeEventListener('scroll', onScroll);
  ...
  return (
    <div className={`min-h-screen bg-background text-foreground ${showBottomBar ? 'pb-16 sm:pb-16' : ''}`}>
      {/* Sticky TOP Bar — visible only in hook section */}
      {!showBottomBar && (
        <div className="sticky top-0 z-50 w-full bg-primary text-primary-foreground shadow-lg">
          ...
          <div className="w-full bg-red-950/60 border-t border-red-900/30 py-1 text-center">
      ...
      {/* Sticky BOTTOM Bar — visible when scrolled past hero image */}
      {showBottomBar && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-primary text-primary-foreground shadow-[0_-4px_20px_rgba(220,38,38,0.3)]">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-2 px-3 py-2 sm:px-6 sm:py-3">
            <div className="flex items-center gap-2 text-xs sm:text-sm">
              <span className="line-through opacity-80">$170</span>
              <span className="hidden sm:inline">expira en:</span>
            ...
            <a href={HOTMART_LINK} className="inline-flex items-center gap-2 rounded-md bg-white px-3 sm:px-4 py-2 text-xs sm:text-sm font-bold text-primary shadow hover:bg-white/90 transition-colors">
              🔥 COMPRAR AHORA — $14 →
            </a>
          </div>
        </div>
      ...
      {proof && (
        <div className={`fixed left-2 right-2 z-[60] mx-auto max-w-[260px] social-proof-card sm:left-4 sm:right-auto ${showBottomBar ? 'bottom-20' : 'bottom-4'}`}>
      ...
      <a
        href="https://wa.link/fjgljb"
        target="_blank"
        rel="noopener noreferrer"
        className={`fixed right-3 z-[60] flex items-center justify-center rounded-full shadow-lg transition-all duration-300 ${showBottomBar ? 'bottom-20' : 'bottom-4'}`}
